...
class DDoS_Monitor(SimpleSwitch13):
    ...
    def _packet_in_handler(self, ev):
        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                self.add_flow(datapath, 1, match, actions, msg.buffer_id, idle_timeout=25)
                return
            else:
    ...
    def _monitor(self):
        while True:
            # --- FIX 2: Giảm thời gian sleep xuống 2s để detect nhanh hơn ---
            hub.sleep(1) 
            if self.datapaths:
                for dp in self.datapaths.values():
                    self._request_stats(dp)
                
                # Chờ một chút để stats kịp về trước khi predict (quan trọng)
                hub.sleep(0.5) 
        ...
        for idx in attack_indices:
            try:
                ...
                if datapath_id in self.datapaths:
                    ...
                    self.logger.info("MITIGATION: BLOCKED ALL TRAFFICS %s -> %s", attacker_ip, victim_ip)
            except Exception as e:

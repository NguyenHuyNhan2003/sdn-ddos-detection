        ...
        if out_port != ofproto.OFPP_FLOOD:
            ...
            if eth.ethertype == ether_types.ETH_TYPE_IP:
                ...
                if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                    self.add_flow(datapath, 1, match, actions, flow_serial_no, msg.buffer_id, idle=8, hard=60)
                    return
                else:
                    self.add_flow(datapath, 1, match, actions, flow_serial_no, idle=8, hard=60)
        data = None
        if msg.buffer_id == ofproto.OFP_NO_BUFFER:
            data = msg.data
